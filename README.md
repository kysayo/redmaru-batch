...
`my-redmaru-app`（Redmine → 社内AIチャット連携のChrome拡張機能）の「AI回答更新」ボタンを、Playwrightで機械的に押して回るバッチ処理。対象チケットの抽出・一覧表示（ドライラン）、拡張機能入りブラウザの起動検証、チケット1件を指定してのボタンクリック＋完了検知、**複数チケットを自動で順次処理する本バッチ（`batch`）** まで実装済み。
...
### 対象チケット全件のAI回答更新（本バッチ）

```
npm run batch
```

`list`と同じ条件（`staleThreshold`、`--threshold=`での上書きも可）で対象チケットを取得し、`update-one`と同じクリック＋ポーリング処理を1件ずつ**逐次**（同時並列はしない）実行する。

1. ブラウザが開いたら手動ログイン → ターミナルでEnterキー（他コマンドと同じ）
2. 対象チケット一覧を表示してから、先頭から順に1件ずつ処理する
3. 1件ごとに成功（AI更新日時の変化を検知）・タイムアウト（150秒）・エラー（ボタンが見つからない等）のいずれかを記録し、**失敗しても処理を止めず次のチケットへ進む**
4. 次のチケットに移る前に5秒待つ（社内AIチャットへの連続送信を避けるため）
5. 全件処理後、成功・タイムアウト・エラーの件数と、失敗したチケットの一覧を表示する

ブラウザウィンドウを閉じるとプロセスが終了する。送信前後の人間による確認ゲートは設けていない（`my-redmaru-app`の単発ボタンと同じ方針、`docs/handoff-ai-answer-automation.md`参照）。
...
- 処理件数が多い場合の実行時間の見積もり表示や、途中経過の永続化（中断・再開）
- 並列処理は社内AIチャット側の挙動が未知数なため見送り中（`docs/spec.md`参照）
